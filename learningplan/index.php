...
if($id==-1){
    ...
    $PAGE->navbar->add( get_string('add_learningplans', 'local_learningplan'));
}elseif($id>1){
    $PAGE->navbar->add( get_string('pluginname', 'local_learningplan'), new moodle_url('/local/learningplan/index.php'));
    $PAGE->navbar->add( get_string('edit_learningplans', 'local_learningplan'));
}else{
    $PAGE->navbar->add( get_string('pluginname', 'local_learningplan'), new moodle_url('/local/learningplan/index.php'));
    $PAGE->navbar->add( get_string('browse_learningplans', 'local_learningplan'));
}
...
echo $OUTPUT->header();
if($id==-1){
    echo "<h2 class='tmhead2'>".get_string('add_learningplans', 'local_learningplan').'</h2>';
}elseif($id>1){
     echo "<h2 class='tmhead2'>".get_string('edit_learningplans', 'local_learningplan').'</h2>';
}else{
     echo "<h2 class='tmhead2'>".get_string('browse_learningplans', 'local_learningplan').'</h2>';
}
...
if($id > 0){
    ...
    $toform->needapproval = $toform->approvalreqd;
    ...
    $enddate = array('month' => $enddate_month, 'day'=> $enddate_day, 'year' => $enddate_year);
     $toform->shortname = $toform->shortname;
    $toform->startdate = $startdate;
    $toform->enddate = $enddate;
    $toform->bands = $toform->band;    
...
if($approval>0){
    ...
    $changestatus->planid = $approval;
    $changestatus->userid = $USER->id;
    $changestatus->approvedby = '';
    $changestatus->approvestatus = '';
    $changestatus->reject_msg = '';
    $changestatus->timecreated = time();
   
    $changestatus->timemodified = time();
    $changestatus->usermodified = $USER->id;
    $check=$DB->record_exists('local_learningplan_approval',array('planid'=>$approval,'userid'=>$USER->id));
    if(!$check){
        $confirm_insert = $DB->insert_record('local_learningplan_approval',$changestatus);
        } 
    
    if($confirm_insert){
        $emailtype="lep_approval_request";
        $status="waiting";
        $planid=$approval;
         $notification=$learningplan_lib->to_send_request_notification($changestatus,$emailtype,$status,$planid);
        
    }
    $return_url_index = new moodle_url('/local/learningplan/view.php',array('id'=>$approval));
    redirect($return_url_index);
}
...
if (has_capability('local/learningplan:create', $systemcontext) || has_capability('local/learningplan:update', $systemcontext)) {
  ...
  /*HERE SENDING THE PARAMETER TO  **NOT** VIEW THE MANAGE PAGE WITH ALL TABS By Ravi_369*/
    $condition="";
