...
require_login();
$PAGE->set_url('/local/learningplan/mylearningplan.php');
...
echo $OUTPUT->header();
echo "<h2 class='tmhead2'>".get_string('my_learningplans', 'local_learningplan').'</h2>';
        ...
        if($startdate_check <= $enddate_check){
            ...
            if ($fromform->submitbutton == 'Save changes') {
                ...
                if($fromform->id > 0){
                    ...
                    //}
                    if(is_array($fromform->costcenterid))  {
                    ...
                    $subsubdeptlist=implode(',',$fromform->sub_sub_department);
...
//if(is_siteadmin()){
    echo $learningplan_renderer->all_enroled_learningplans();
//}
