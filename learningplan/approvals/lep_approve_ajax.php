...
require_once($CFG->dirroot . '/local/learningplan/approvals/includes.php');
...
// coe published the exam
  case 'request_user':
		...
		if($id==0){
			...
			$sql = "SELECT f.* ,fa.id as fapprovalid FROM {local_learningplan} f JOIN
            {local_learningplan_approval} fa ON f.id = fa.planid where fa.planid=$id";
   ...
   $enrol_confirm = new facetoface_enrolment();
    ...
    $draftcourseheadig = '<div class="pl_newtoggle">'.html_writer:: tag('h5',$drcourse->name,array()).'</div>';
        ...
        $user_Requested=$DB->get_records_sql("SELECT fa.id as f2fuserid
                                FROM {local_learningplan_approval} fa JOIN {local_userdata} ud ON fa.userid=ud.userid
                                JOIN {user} u ON ud.userid=u.id
                                WHERE fa.approvestatus = 0 AND fa.planid=$drcourse->id AND u.deleted=0 AND u.suspended=0");
        

        $user_Approved=$DB->get_records_sql("SELECT fa.id as f2fuserid
                                FROM {local_learningplan_approval} fa JOIN {local_userdata} ud ON fa.userid=ud.userid
                                JOIN {user} u ON ud.userid=u.id
                                WHERE fa.approvestatus = 1 AND fa.planid=$drcourse->id AND u.deleted=0 AND u.suspended=0");
				        
				$user_rejected=$DB->get_records_sql("SELECT fa.id as f2fuserid
                                FROM {local_learningplan_approval} fa JOIN {local_userdata} ud ON fa.userid=ud.userid
                                JOIN {user} u ON ud.userid=u.id
                                WHERE fa.approvestatus = 2 AND fa.planid=$drcourse->id AND u.deleted=0 AND u.suspended=0");
...
//
  case 'req_data':
		...
		//$cat='new';
		$enrol_confirm = new facetoface_enrolment();
		$enrol_users = $enrol_confirm->planEnrolment($facetofaceid,$cat);
