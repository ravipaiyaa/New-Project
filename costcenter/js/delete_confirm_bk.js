 ...
 */
M.util.tmahendra_show_confirm_dialog = function(e, args) {
    ...
    YUI().use('moodle-core-notification-confirm', function(Y) {
        ...
        confirmationDialogue.on('complete-yes', function(e) {
            ...
            if (target.test('a')) {
                 window.location = ''+target.get('href')+'?id='+args.callbackargs.id+'&delete=1&confirm=1&sesskey='+M.cfg.sesskey+'&'+args.callbackargs.extraparams+'';
            } else {
...
function assign_manager(costcenterid) {
        $("#employeedialog"+costcenterid+"").hide();
    YUI().use('node','transition', function(Y) {
	node = Y.one("#dialog"+costcenterid+"");
            node.toggleView();
    });
    $("div.dialog1").css("background-color", "#b5e9f6");
     $("div.dialog1").css("border","none");
     $("div.dialog1").css("box-shadow","none");
     $("#employeedialog"+costcenterid+"").parent("td").toggleClass("td");
}
function assign_employee(costcenterid) {
        $(".assign-user-select"+costcenterid+"").select2();
    $("#dialog"+costcenterid+"").hide();
    YUI().use('node','transition', function(Y) {
	node = Y.one("#employeedialog"+costcenterid+"");
            node.toggleView();
    });
     $("div.dialog1").css("background-color", "#b5e9f6");
     $("div.dialog1").css("border","none");
