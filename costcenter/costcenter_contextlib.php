...
/** costcener context level -  one instance for each costcenter */
define('CONTEXT_COSTCENTER', 27);

/**
 * Costcenter context class
 *
 * @package   core_access
 * @category  access
...
class context_costcenter extends context {
     ...
     *
     * @param boolean $withprefix whether to prefix the name of the context with Costcenter
     * @param boolean $short does not apply to costcenter
    ...
    public function get_context_name($withprefix = true, $short = false) {
        ...
        if ($costcenter = $DB->get_record('local_costcenter', array('id' => $this->_instanceid))) {
            if ($withprefix) {
                $name = get_string('pluginname', 'local_costcenter') . ': ';
            }
            $name .= format_string($costcenter->name, true, array('context' => $this));
        }
        return $name;
    }
    ...
    public function get_url() {
        return new moodle_url('/local/costcenter/index.php');
    }
    ...
    /**
     * Returns immediate child contexts of costcenter and all subcostcenters,
     * children of subcostcenter and courses are not returned.
     *
     * @return array
     */
...
//$contextconfig = new custom_contexts_config();
//$contextconfig->costcenter_custom_contexts_setup();
...
/**
 * this function used to set the global variable COSTCENTER
 *
 * @package custom
 * @method setup_global_costcenter_info
 * @return object (loggedin user costcenter info)
 * */
function setup_global_costcenter_info() {
    global $CFG, $DB, $USER, $OUTPUT;

    $costcenterinfo = $DB->get_record('local_userdata', array('userid' => $USER->id));
   
    if (isset($costcenterinfo->costcenterid)) {
        $cc_courses = $DB->get_records('course', array('costcenter' => $costcenterinfo->costcenterid, 'visible' => 1));
        //print_object( $cc_courses);
        foreach ($cc_courses as $courses) {
            $response_courses[] = $courses->id;
        }
        if (!empty($response_courses))
            $costcenterinfo->courses = $response_courses;
        ...
        if (!empty($response))
            $costcenterinfo->enroledcourses = $response;
    }
